...
export interface UpdateUnitDcRequest {
  deliveryDate: string
  sentToPersonName?: string
  notes?: string
...
export interface UpdateAccessoryDcRequest {
  deliveryDate: string
  sentToPersonName?: string
  notes?: string
  items: { id: string; quantity: number }[]
...
export const unitDcApi = {
  ...
  update: (id: string, data: UpdateUnitDcRequest) => api.patch<UnitDc>(`/api/v1/unit-dc/${id}`, data).then(r => r.data),
  confirm: (id: string) => api.post<UnitDc>(`/api/v1/unit-dc/${id}/confirm`).then(r => r.data),
  delete: (id: string) => api.delete(`/api/v1/unit-dc/${id}`),
}
...
export const accessoryDcApi = {
  ...
  update: (id: string, data: UpdateAccessoryDcRequest) => api.patch<AccessoryDc>(`/api/v1/accessory-dc/${id}`, data).then(r => r.data),
  confirm: (id: string) => api.post<AccessoryDc>(`/api/v1/accessory-dc/${id}/confirm`).then(r => r.data),
  delete: (id: string) => api.delete(`/api/v1/accessory-dc/${id}`),
}
