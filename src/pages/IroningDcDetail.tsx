...
import { ironingDcApi, type IroningDc } from '../api/ironingDc'
import { printIroningDc } from '../utils/printDc'
...
export default function IroningDcDetail() {
  const { id } = useParams<{ id: string }>()
  const navigate = useNavigate()
  const { data: doc, loading, reload } = useApiData(
    () => id ? ironingDcApi.getById(id) : Promise.resolve(null as IroningDc | null),
    [id]
  )
  ...
  const doSaveEdit = async () => {
    ...
    setBusy(true)
    try {
      await ironingDcApi.update(doc.id, { deliveryDate: editDate, sentToPersonName: editSentTo || undefined, notes: editNotes || undefined, items: doc.items.map(i => ({ id: i.id, quantity: editQtys[i.id] ?? i.quantity })) })
  ...
  const doConfirmDc = async () => {
    if (!doc) return
    setBusy(true)
    try { await ironingDcApi.confirm(doc.id); reload() }
    ...
    <div className="page">
      ...
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="btn btn-secondary" onClick={() => printIroningDc(doc)}>Print DC</button>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        ...
        <div style={{ padding: '16px 20px', display: 'grid', gridTemplateColumns: '1fr 1fr', rowGap: 14, columnGap: 40 }}>
          ...
          {editing ? <div><div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4 }}>Ironing Date</div><input type="date" className="form-control" value={editDate} onChange={e => setEditDate(e.target.value)} /></div> : <InfoRow label="Ironing Date" value={doc.deliveryDate} />}
