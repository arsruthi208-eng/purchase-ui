...
import { checkingDcApi, type CheckingDc, type CreateCheckingDcRequest } from '../api/checkingDc'
...
type Line = CreateCheckingDcRequest['items'][0]
const emptyLine = (): Line => ({ styleId: '', gender: 'BOYS', standard: '', quantity: 0 })

export default function CheckingDc() {
  ...
  const [form, setForm] = useState<CreateCheckingDcRequest>({
    stitchingUnitId: '', schoolOrderId: '', checkDate: today(), items: [emptyLine()],
  })
  ...
  const openCreate = () => {
    setForm({ stitchingUnitId: units[0]?.id ?? '', schoolOrderId: '', checkDate: today(), items: [emptyLine()] })
  ...
  const onSchoolOrderChange = async (id: string) => {
    ...
    try {
      ...
      const autoItems = detail.items.map(it => ({
        styleId: it.styleId, gender: it.gender, standard: it.standard, quantity: it.quantity,
      }))
      setForm(p => ({ ...p, schoolOrderId: id, items: autoItems.length > 0 ? autoItems : [emptyLine()] }))
    } catch { showError('Load Failed', 'Could not load school order details.') }
  ...
  const save = async () => {
    if (!form.schoolOrderId || !form.stitchingUnitId || !form.checkDate) {
    ...
    try { await checkingDcApi.create(form); setShowForm(false); load() }
    catch (e: any) { setError(e.message) } finally { setSaving(false) }
  }

  const confirmDc = (id: string) => {
    showConfirm('Confirm Checking DC', 'Garments will be marked as QC-passed. This cannot be undone.', async () => {
      try { await checkingDcApi.confirm(id); load() }
      catch (e: any) { isForbiddenError(e) ? showError('Access Denied', PERMISSION_DENIED_MSG) : showError('Confirm Failed', e.message) }
    })
  }

  const deleteDc = (d: CheckingDc, e: React.MouseEvent) => {
    e.stopPropagation()
    showConfirm(
      'Delete Checking DC',
      `Permanently delete ${d.dcNumber}? This cannot be undone.`,
      async () => {
        try { await checkingDcApi.delete(d.id); load() }
  ...
  const sorted = [...filtered].sort((a, b) =>
    sortDesc ? b.checkDate.localeCompare(a.checkDate) : a.checkDate.localeCompare(b.checkDate)
  )
  ...
  return (
    <div className="page">
      ...
      <div className="page-header">
        <div>
          <div className="page-title">Checking DC</div>
          <div className="page-subtitle">{items.length} quality-check notes</div>
        </div>
        <button className="btn btn-primary" onClick={openCreate}><Plus size={16} /> New Checking DC</button>
      </div>

      {showForm && (
        <Modal title="New Checking DC" onClose={() => setShowForm(false)} width={680}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <div className="form-group">
              ...
              <div style={{ padding: '8px 10px', borderRadius: 6, background: 'var(--bg)', border: '1px solid var(--border)', fontSize: 14, fontWeight: 500 }}>
                {selectedOrder
                  ? <>{selectedOrder.schoolName}<span style={{ fontSize: 11, color: 'var(--text-muted)', marginLeft: 6 }}>from school order</span></>
          ...
          <div style={{ marginBottom: 6, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            {selectedOrder && <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>Auto-filled from {selectedOrder.orderNumber} — qty is editable</span>}
          </div>
